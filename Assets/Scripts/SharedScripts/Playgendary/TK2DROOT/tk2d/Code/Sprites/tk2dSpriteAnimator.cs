...
public class tk2dSpriteAnimator : MonoBehaviour
{
	...
	virtual public tk2dBaseSprite Sprite {
		get {
			if (_sprite == null) {
				...
				if (_sprite == null) {
					CustomDebug.LogError("Sprite not found attached to tk2dSpriteAnimator.");
				}
			}
			return _sprite;
		}
	...
	tk2dSpriteAnimationClip GetClipByNameVerbose(string name) {
		if (library == null) {
			CustomDebug.LogError("Library not set");
			return null;
		}
		else {
			tk2dSpriteAnimationClip clip = library.GetClipByName( name );
			if (clip == null) {
				CustomDebug.LogError("Unable to find clip '" + name + "' in library");
				return null;
			}
			else {
				return clip;
...
#region PlayFrom
	...
	public void Play(tk2dSpriteAnimationClip clip, float clipStartTime, float overrideFps)
	{
		...
		{
			CustomDebug.LogError("Calling clip.Play() with a null clip");
	...
	public int CurrentFrame {
		get {
			switch (currentClip.wrapMode) {
				...
				default: {
					CustomDebug.LogError("Unhandled clip wrap mode");
	...
	void ClipNameError(string name) {
		CustomDebug.LogError("Unable to find clip named '" + name + "' in library");
	}

	void ClipIdError(int id) {
		CustomDebug.LogError("Play - Invalid clip id '" + id.ToString() + "' in library");
	}
