			...
			foreach (string line in lines)
			{
				if (line.Trim().Length != 0)
				{
					...
					if (splitIndex != -1)
					{
						...
						{
							CustomDebug.LogError("Parse error in line " + lineNumber.ToString());
							return false;
						}
					...
					if (spriteId == -1)
					{
						CustomDebug.LogError(string.Format("Unable to find sprite '{0}' in sprite collection", spriteName));
						return false;
					}
