...
namespace tk2dEditor.Font
{
	...
	public static class Builder
	{
		public static Info ParseBMFont(string path)
		{
			...
			{
				CustomDebug.LogError("Font parsing returned 0 characters, check source bmfont file for errors");
				return null;
			}
		...
		public static bool BuildFont(Info fontInfo, tk2dFontData target, float scale, int charPadX, bool dupeCaps, bool flipTextureY, Texture2D gradientTexture, int gradientCount)
		{
			...
			foreach (var theChar in fontInfo.chars)
			{
				if (theChar.id > maxUnicodeChar)
				{
					...
					CustomDebug.LogError("Unicode character id exceeds allowed limit: " + theChar.id.ToString() + ". Skipping.");
					continue;
				}
			...
			{
				var curChars = new tk2dFontChar[maxCharId + 1];
				for (int i = 0; i <= maxCharId; ++i)
				{
					curChars[i] = chars[i];
					if (curChars[i] == null)
					{
						curChars[i] = nullChar; // zero everything, null char
					}
				}

				target.SetArray(curChars);
			}
