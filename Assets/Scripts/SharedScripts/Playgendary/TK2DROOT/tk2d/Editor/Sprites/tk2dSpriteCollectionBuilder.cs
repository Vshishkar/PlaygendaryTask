...
public class tk2dSpriteCollectionBuilder
{
	...
	{
		// Revert default padMode from extend to balckZeroAlpha
    ...
    static Texture2D ProcessTexture(tk2dSpriteCollection settings, int texIndex, tk2dSpriteCollectionDefinition.Pad padMode, bool disableTrimming, bool isInjectedTexture, bool isDiced, Texture2D srcTex, int sx, int sy, int tw, int th, ref SpriteLut spriteLut, int padAmount)
	{
...
//                    CustomDebug.Log("this scale: " + thisScale);
            ...
            gen.spriteCollection.widthTextures = new int[0];
            gen.spriteCollection.heightTextures = new int[0];
		...
		for (int i = 0; i < gen.textureParams.Length; ++i)
		{
			...
			if (gen.textureParams[i].dice)
			{
				...
				for (int sx = 0; sx < srcTex.width; sx += diceUnitX)
				{
					for (int sy = 0; sy < srcTex.height; sy += diceUnitY)
					{
                        ...
                        Texture2D dest = ProcessTexture(gen, i, tk2dSpriteCollectionDefinition.Pad.Extend, gen.textureParams[i].disableTrimming, false, true, srcTex, sx, sy, tw, th, ref diceLut, GetPadAmount(gen, i));
						if (dest)
						{
			...
			else
			{
				...
				if (!lut.isDuplicate)
				{
                    ...
                    Texture2D dest = ProcessTexture(gen, i, gen.textureParams[i].pad, gen.textureParams[i].disableTrimming, false, false, currentTexture, 0, 0, currentTexture.width, currentTexture.height, ref lut, GetPadAmount(gen, i));
					if (dest == null)
					{
		...
		if (gen.fonts != null)
		{
			for (int i = 0; i < gen.fonts.Length; ++i)
			{
				...
				foreach (var c in fontInfo.chars)
				{
                    ...
                    Texture2D dest = ProcessTexture(gen, i, tk2dSpriteCollectionDefinition.Pad.BlackZeroAlpha, false, true, false,
