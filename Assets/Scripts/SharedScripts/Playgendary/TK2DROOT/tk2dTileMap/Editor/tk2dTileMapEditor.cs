...
public class tk2dTileMapEditor : Editor, ITileMapEditorHost
{
	...
	void DrawTileDataSetupPanel()
	{
        ...
        Rect rect = brushRenderer.DrawBrush(tileMap, editorData.paletteBrush, editorData.brushDisplayScale, true, editorData.paletteTilesPerRow, innerRect);
	...
	void DrawSettingsPanel()
	{
		...
		if (Foldout(ref editorData.setupMode, tk2dTileMapEditorData.SetupMode.TileProperties, "Tile Properties"))
		{
            ...
            float zoomFactor = (EditorGUIUtility.currentViewWidth - 32.0f) / (spritePixelSize.x * 2.0f);
		...
		if (Foldout(ref editorData.setupMode, tk2dTileMapEditorData.SetupMode.PaletteProperties, "Palette Properties"))
		{
			...
			innerRect.height *= editorData.brushDisplayScale;
			tk2dGrid.Draw(innerRect);
            brushRenderer.DrawBrush(tileMap, editorData.paletteBrush, editorData.brushDisplayScale, true, editorData.paletteTilesPerRow, innerRect);
			EndHScrollView();
	...
	void DrawPaintPanel()
	{
		...
		if (!showLoadSection)
		{
			...
			if (editorData.showBrush)
			{
				...
				innerRect.height *= editorData.brushDisplayScale;
				tk2dGrid.Draw(innerRect);
                brushRenderer.DrawBrush(tileMap, editorData.activeBrush, editorData.brushDisplayScale, false, editorData.paletteTilesPerRow, innerRect);
				EndHScrollView();
				EditorGUILayout.Separator();
			}
		}
