...
public class tk2dScratchpadGUI {
	...
	public void DrawGUI() {
		...
		if (currentScratchpad != null) {
            ...
            Vector2 sizeSceneView = new Vector2(Screen.width, Screen.height);
            if (SceneView.currentDrawingSceneView != null)
            {
                sizeSceneView = SceneView.currentDrawingSceneView.position.size;
            }
            canvasScrollPos = EditorGUILayout.BeginScrollView(canvasScrollPos, GUILayout.Width(Mathf.Min(scratchW * tileSize.x * scratchZoom + 24.0f, sizeSceneView.x - 190.0f)));
			...
			if (Event.current.type == EventType.Repaint) {
				if (brushRenderer != null) {
                    brushRenderer.DrawBrushInScratchpad(currentScratchpad.CanvasBrush, canvasMatrix, false, padRect);
					if (workingBrush != null && workingHere) {
                        brushRenderer.DrawBrushInScratchpad(workingBrush, canvasMatrix, true, padRect);
					}
				}
