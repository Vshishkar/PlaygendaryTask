...
public class tk2dSpritePickerPopup  : EditorWindow
{
	class PresentationParams
	{
        ...
        public void Update(int numTiles, int widthWindow) {
			tileSizeInt = TileSize;
            int w = widthWindow - scrollbarWidth - border * 2;
	...
	void OnGUI()
	{
        ...
        int widthWindow = (int)this.position.width;
        int heigthWindow = (int)this.position.height;

        presentParams.Update(selectedDefinitions.Count, widthWindow);
        ...
        GUILayout.BeginArea(new Rect(0, 0, widthWindow, heigthWindow));
