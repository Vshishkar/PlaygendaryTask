...
public class tk2dFontEditor : Editor 
{
	...
	public override void OnInspectorGUI()
	{
			...
			if (tex && IsTextureCompressed(tex))
			{
					...
					)) != -1)
				{
					if (buttonPressed == 0)
					{
                        ...
                        ConvertTextureToFormat(tex, TextureImporterFormat.RGBA16);
                        #else
					...
					{
                        #if UNITY_5_5_OR_NEWER
                        ConvertTextureToFormat(tex, TextureImporterFormat.RGBA32);
                        #else
			...
			(gen.gradientTexture.format != TextureFormat.ARGB32 && gen.gradientTexture.format != TextureFormat.RGB24 && gen.gradientTexture.format != TextureFormat.RGBA32))
		{
			...
			{
                #if UNITY_5_5_OR_NEWER
                ConvertTextureToFormat(gen.gradientTexture, TextureImporterFormat.RGBA32);
                #else
