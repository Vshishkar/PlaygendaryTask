...
namespace tk2dEditor.Atlas
{
	...
	public class Builder
	{
        ...
        public int Build(bool forceBottomLeft)
		{
			// Initialize
...
//					allUsed = binPacker.Insert(currRects, MaxRectsBinPack.FreeRectChoiceHeuristic.RectBestAreaFit);
                    MaxRectsBinPack binPacker;
                    if (forceBottomLeft)
                    {
						binPacker = new MaxRectsBinPack(thisCellW, thisCellH, allowRotation);
                        List<RectSize> activeRects = new List<RectSize>(currRects);
                        allUsed = binPacker.Insert(activeRects, MaxRectsBinPack.FreeRectChoiceHeuristic.RectBottomLeftRule);
                    }
                    else
                    {
                        binPacker = FindBestBinPacker(thisCellW, thisCellH, ref currRects, ref allUsed);
                    }
