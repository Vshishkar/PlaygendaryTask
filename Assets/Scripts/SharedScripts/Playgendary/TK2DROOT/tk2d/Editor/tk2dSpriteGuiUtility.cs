...
public static class tk2dSpriteGuiUtility 
{
	...
	static void BuildLookupIndex(bool force)
	{
		...
		allSpriteCollections.Sort(new tk2dEditor.Shared.Tk2dSpriteCollectionIndexNaturalComparer());
	...
	public static tk2dSpriteCollectionData SpriteCollectionList(tk2dSpriteCollectionData currentValue) {
		...
		else
		{
			...
			{
				CustomDebug.LogError("Unable to find sprite collection. This is a serious problem.");
			...
			else
			{
				...
				if (newSelection != currentSelection)
				{
					...
					if (newData == null)
					{
						CustomDebug.LogError("Unable to load sprite collection. Please rebuild index and try again.");
					}
