...
public abstract class tk2dBaseSprite : tk2dBaseMesh, tk2dRuntime.ISpriteCollectionForceBuild
{
	...
	{
		var sprite = CurrentSprite;
        Bounds result = (sprite != null) ? 
            (new Bounds(new Vector3(sprite.untrimmedBoundsData[0].x * _scale.x, sprite.untrimmedBoundsData[0].y * _scale.y, sprite.untrimmedBoundsData[0].z * _scale.z),
	...
	public virtual void EditMode__CreateCollider()
	{
		...
		tk2dSpriteDefinition sprite = CurrentSprite;
        if ((sprite == null) || (sprite.colliderType == tk2dSpriteDefinition.ColliderType.Unset))
			return;
    ...
    {
        #if UNITY_EDITOR
        if (CachedRenderer != null && Collection != null && CachedRenderer.sharedMaterial == null && Collection.inst.needMaterialInstance) {
			ForceBuild();
		}
        #endif
    }
