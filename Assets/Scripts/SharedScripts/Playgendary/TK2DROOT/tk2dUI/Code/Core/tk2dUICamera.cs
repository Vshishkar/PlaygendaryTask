...
public class tk2dUICamera : MonoBehaviour {
	...
	void OnEnable() {
		if (GetComponent<Camera>() == null) {
			CustomDebug.LogError("tk2dUICamera should only be attached to a camera.");
			enabled = false;
			return;
		}

		if (!GetComponent<Camera>().orthographic && raycastType == tk2dRaycastType.Physics2D) {
			CustomDebug.LogError("tk2dUICamera - Physics2D raycast only works with orthographic cameras.");
			enabled = false;
			return;
		}
